...
namespace UnityEditor.ProBuilder
{
	class ElementGroup
	{
		...
		public ElementGroup(IEnumerable<int> indices, Vector3 position, Quaternion rotation)
		{
	...
	class VertexManipulationToolSelection
	{
		...
		public IEnumerable<ElementGroup> elementGroups
		{
			get { return m_Selection; }
		}

		public VertexManipulationToolSelection(ProBuilderMesh mesh, List<ElementGroup> groups)
		{
			m_Mesh = mesh;
			m_Selection = groups;
		}

		public static IEnumerable<VertexManipulationToolSelection> GetVertexManipulationToolSelection(
			IEnumerable<ProBuilderMesh> meshes,
			SelectMode selectMode,
			PivotPoint pivot,
			HandleOrientation orientation,
			VertexComparison vertexCompareFunction)
        {
            List<VertexManipulationToolSelection> selection = new List<VertexManipulationToolSelection>();
            bool collectCoincident = vertexCompareFunction == VertexComparison.Common;
            ...
            foreach (var mesh in meshes)
            {
                ...
                switch (pivot)
                {
                    case PivotPoint.IndividualOrigins:
                    {
                        if (selectMode.ContainsFlag(SelectMode.Vertex | SelectMode.TextureVertex))
                        {
                            foreach (var list in GetVertexSelectionGroups(mesh, collectCoincident))
                            {
                                ...
                                var rot = PHandleUtility.GetVertexRotation(mesh, orientation,
                                    list);
                        ...
                        else if (selectMode.ContainsFlag(SelectMode.Edge | SelectMode.TextureEdge))
                        {
                            ...
                            {
                                var bounds = Math.GetBounds(mesh.positionsInternal, list);
                                var rot = PHandleUtility.GetEdgeRotation(mesh, orientation, list);
                        ...
                        else if (selectMode.ContainsFlag(SelectMode.Face | SelectMode.TextureFace))
                        {
                            ...
                            {
                                var bounds = Math.GetBounds(mesh.positionsInternal, list);
                                var rot = PHandleUtility.GetFaceRotation(mesh, orientation, list);
                                List<int> indices;
                    ...
                    case PivotPoint.ActiveElement:
                    {
                        ...
                        if (selectMode.ContainsFlag(SelectMode.Face | SelectMode.TextureFace))
                        {
                            ...
                            if (face != null)
                            {
                                position = trs.MultiplyPoint3x4(Math
                                    .GetBounds(mesh.positionsInternal, face.distinctIndexesInternal).center);
                                rotation = PHandleUtility.GetFaceRotation(mesh, orientation,
                                    new Face[] { face });
                            }
                        }
                        else if (selectMode.ContainsFlag(SelectMode.Edge | SelectMode.TextureEdge))
                        {
                            ...
                            {
                                position = trs.MultiplyPoint3x4(Math
                                    .GetBounds(mesh.positionsInternal, new int[] { edge.a, edge.b }).center);
                                rotation = PHandleUtility.GetEdgeRotation(mesh, orientation,
                                    new Edge[] { edge });
                            }
                        }
                        else if (selectMode.ContainsFlag(SelectMode.Vertex | SelectMode.TextureVertex))
                        {
                            ...
                            if (vertex > -1)
                            {
                                ...
                                rotation = PHandleUtility.GetVertexRotation(mesh, orientation,
                                    new int[] { vertex });
                            }
                        }
                ...
                selection.Add(new VertexManipulationToolSelection(mesh, groups));
            }
